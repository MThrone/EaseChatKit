...
@interface EaseInputBar()<UITextViewDelegate>
...
- (void)_setupSubviews
{
    ...
    self.inputView.backgroundColor = _viewModel.inputBarBgColor;
...
- (void)moreButtonAction:(UIButton *)aButton
{
    ...
    if (self.viewModel.extFuncModel.extFuncStyle == EasePopupView) {
    ...
    if (aButton.selected){
        if(self.extFunctionView) {
            self.currentMoreView = self.extFunctionView;
            [self addSubview:self.extFunctionView];
            [self.extFunctionView Ease_makeConstraints:^(EaseConstraintMaker *make) {
                make.left.equalTo(self);
                make.right.equalTo(self);
                make.bottom.equalTo(self).offset(-EaseVIEWBOTTOMMARGIN);
